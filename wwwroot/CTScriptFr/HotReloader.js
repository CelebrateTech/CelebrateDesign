class HotReloader {
    constructor(obj) {
        ...
        this.pageLoaderHtmlId = obj.pageLoaderHtmlId;
    }
    ...
    async _updateContent(doc) {
        const newContent = doc.querySelector(`#${this.bodyId}`);
        if (!newContent) {
            throw new Error('Could not find RenderBody element in new page');
        }

        const currentContent = document.querySelector(`#${this.bodyId}`);
        if (currentContent) {
        ...
        // Scroll to top
        this._scrollToTop(currentContent);
        this.hideLoader();
    }
    ...
    showLoader() {
        const loader = document.getElementById(this.pageLoaderHtmlId);
        if (loader) {
            loader.style.display = 'flex';
        } else {
            console.error(`Element with ID '${this.pageLoaderHtmlId}' not found.`);
        }
    }
